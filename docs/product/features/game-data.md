...
- `game_data_items` 按资料目录与物品 ID 唯一，保存常用检索字段和完整 JSON 属性。
- `php artisan game-data:import-items --all` 将客户端与 Renewal 服务端快照导入数据库；HTTP API 只查询数据库，不在请求期间解析大体积 JSON。
- 导入以 `(resource_type, source, ruleset, source_version)` 定位资料目录，并按 `(game_data_catalog_id, item_id)` 执行 upsert；每轮同步会删除同一版本中已不存在的记录，重复执行结果一致。
